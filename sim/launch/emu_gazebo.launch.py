...
def generate_launch_description():
    ...
    world_file_path = os.path.join(pkg_sim_dir, 'worlds')
    ...
    start_gazebo_server = IncludeLaunchDescription(
        ...
        launch_arguments={
            'world': PathJoinSubstitution([world_file_path, LaunchConfiguration('world')]),
            'verbose': 'true',
    ...
    load_right_leg_controller = Node(
        package='controller_manager',
        executable='spawner',
        arguments=['right_leg_position_controller'],
        output='screen',
        parameters=[{'use_sim_time': LaunchConfiguration('use_sim_time')}]
    )
    ...
    load_head_controller = Node(
        package='controller_manager',
        executable='spawner',
        arguments=['neck_position_controller'],
        output='screen',
        parameters=[{'use_sim_time': LaunchConfiguration('use_sim_time')}]
    )
    ...
    return LaunchDescription([
        ...
        # Controllers (delayed start via event handlers only)
